...
var docNpcs = function(doc) {
	emit(doc._id, {"npcs": doc.docs});
}

var docNpcValues = function(doc) {
	doc.docs.forEach(function(npc) {
		emit(npc.npc_id, {"npc_id": npc.npc_id, "npc_name": npc.npc_name, "value": npc.value});
	});
}

var docNpcQAs = function(doc) {
	doc.docs.forEach(function(npc) {
		emit(npc.npc_id, {"npc_id": npc.npc_id, "npc_name": npc.npc_name, "question": npc.question, "answer": npc.answer});
	});
}
...
router.get('/designDoc', function(request, response) {
	...
	}, '_design/gameState', function(error, body) {
		if (!error) {
			console.log(body);
			response.send(body);
		} else {
...
router.get('/view03', function(request, response) {
	'use strict';
	console.log("view Called: " + JSON.stringify(request.query));
	...
	nanoDb.view(request.query.designDoc, request.query.view,
			function(err, body) {
				if (!err) {
					console.log("View retrieved: " + JSON.stringify(body));

					var result = [];
					body.rows.forEach(function(docs) {
						docs.value.npcs.forEach(function(npc) {
							result.push(npc);
							console.log(npc.id);
						})
					});
