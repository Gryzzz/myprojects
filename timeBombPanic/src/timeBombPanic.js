...
//var gameOver = document.querySelector("Won");
...
[
  ...
  [3,1,2,1,2,1,2,1,2,1,1,3],
  [3,1,1,2,1,1,1,1,1,1,1,3],
  [3,1,1,1,1,2,1,1,2,1,1,3],
  [3,1,2,1,2,2,1,2,2,2,1,3],
        ...
        switch (currentTile)
        {
          ...
          case BOMB:
            ...
            case KEY:
                ...
                key = Object.create(spriteObject);
                key.sourceX = 64;
                key.sourceY = 64;
                key.x = column * SIZE ;
                key.y = row * SIZE;
...
function playGame()
{
  ...
  {
      if (alien.y <= sprites[exitPosition].y + 10
          && alien.y >= sprites[exitPosition].y - 10
          && IhaveKey
          )
      {
          sprites[exitPosition].sourceX = alien.sourceX;
          sprites[exitPosition].sourceY = alien.sourceY;
          alien.visible = false;
          endGame();
      }
      else
  ...
  for(var i = 0; i < bombs.length; i++)
  {
    ...
    if(hitTestCircle(alien, bomb) && bomb.visible)
    {
      bomb.visible = false;
      bombsDefused++;
      if(bombsDefused === bombs.length-1)
      {
          key.visible = true;
      }
      if (bombsDefused === bombs.length) {
          sprites[exitPosition].sourceX = 128;
          sprites[exitPosition].sourceY = 64;
          alien.sourceY = 64;
          IhaveKey = true;
...
function endGame()
{
 ...
 // gameOverMessage.visible = true;
  ...
  if(!alien.visible)
  {
      document.getElementById('Won').innerHTML = "<strong> The Game is Over! </strong>" + "<br>" + " It appears that you won. Must be a beginner's luck...";
  }
  else
  {
      document.getElementById('Won').innerHTML = "<strong> The Game is Over! </strong>" + "<br>" + " It appears that you didn't make it on time and now you lost. Good luck next time!";
