...
handlebars.registerHelper('states', function(context, block) {
	...
	for (var i = 0, j = context.length; i < j; i++) {
		if (context[i].capital !== undefined) {
			ret = ret + '<li>' + context[i].name + ': ' + context[i].capital
					+ '</li>';
		} else if (context[i].ISBN !== undefined)

		{
			ret = ret + '<li>' + context[i].name + ': ' + context[i].author
					+ '</li>';
		} else {
			ret = ret + '<li>' + context[i].name + ': '
					+ context[i].mostPopulousCity + '</li>';
		}
	}
	return ret + '</ul>';
});
